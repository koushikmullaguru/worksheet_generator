              ...
              {isEditing ? (
                ...
                <>
                  <div
                    ...
                    dangerouslySetInnerHTML={{ __html: renderAllEquations(question.text) }}
                  />

                  {/* CRITICAL: This section correctly renders the Base64 Data URI */}
                  {question.images && question.images.length > 0 && (
                    <div
                      className="relative w-40 h-24 rounded-lg overflow-hidden border border-border cursor-pointer hover:ring-2 hover:ring-primary transition-all"
                      onClick={() => setImageOpen(true)}
                    >
                      <img
                        // The Base64 string is placed here directly
                        src={question.images[0]}
                        alt="Question diagram"
                        className="w-full h-full object-cover"
                      />
                  ...
                  {(question.options && question.options.length > 0) && (
                    <div className="space-y-2 pl-4">
                      {question.options.map((option, i) => (
                        <div key={i} className="flex items-start gap-2">
                          <span className="text-muted-foreground min-w-[20px]">{String.fromCharCode(65 + i)}.</span>
                          <span dangerouslySetInnerHTML={{ __html: renderAllEquations(option) }} />
                        </div>
                      ))}
                    </div>
                  )}
                          ...
                          >
                            ...
                            <div>
                              ...
                              <span
                                ...
                                dangerouslySetInnerHTML={{ __html: renderAllEquations(question.explanation) }}
                              />
                            </div>
                          </motion.div>
                        </CollapsibleContent>
